...
push(_r+1*SIZE,_c+2*SIZE,-(_sign)*(_a)[0]); push(_r+2*SIZE,_c+2*SIZE,_diag); };
	...
	// data
	for(int l=0; l<len; l++) 
		fprintf(file,"%d %d %.*" RF "g\n",data[l].r+1,data[l].c+1,DIGITS+2,RT(data[l].v));
	// footer
	fprintf(file,"\n\n");
	free(data);
};
...
void oct_save_linear(FILE* file) {
	...
	if(nonuniform_field) {
		for3(j) forlla(u,x,y,z)
			fprintf(file,"%.*" RF "g\n",DIGITS+2,RT(-nonuniform_field[j+3*INDEX(u,x,y,z)]));	
	} else {
		for3(j) forlla(u,x,y,z)
			fprintf(file,"%.*" RF "g\n",DIGITS+2,RT(-magnetic_field[j]));	
	};
	fprintf(file,"\n\n");	
};
